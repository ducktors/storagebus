...
test('@storagebus/storage', async (t) => {
  await t.test('sanitize key', async (t) => {
    await t.test(
      'trying to sanitize values that are not strings, not numbers or empty strings results in an error',
      () => {
    ...
    await t.test('numbers should be converted to strings', () => {
      assert.equal(sanitize(123), '123')
      assert.equal(sanitize(Number(123)), '123')
    })

    await t.test(
      '"123#@%$^&@456!-)+=*_" should be sanitized to "123456!-)*_"',
      () => assert.equal(sanitize('123#@%$^&@456!-)+=*_'), '123456!-)*_'),
    )

    await t.test('"áêīòü" should be sanitized to "aeiou"', () =>
      assert.equal(sanitize('áêīòü'), 'aeiou'),
    )

    await t.test(
      'spaces are replaced with the provided separator or the default',
      () => {
    ...
    await t.test(
      'an error is thrown if the requested separator is not valid',
      () => {
